...
class Sine2(ReparametrizedModel):
    """Sine Squared model according to:
    ...
    ```
    sine^2(x) + y0 = 0.5 * (1 - cos(2 * x)) + y0 = (y0 + 0.5) - 0.5 * sin(pi/2 - 2 * x)
    ```
    """

    def __init__(self):
        model = ScaledModel(model=Sinusoid(), x_scale=-2, x_offset=np.pi / 2.0)

        super().__init__(
            model=model,
            new_params={
                "y0": utils.param_like(model.parameters["y0"]),
                "a": utils.param_like(model.parameters["a"]),
            },
            bound_params=["y0", "a"],
        )
    ...
    @staticmethod
    def bound_param_values(param_values: Dict[str, float]) -> Dict[str, float]:
        return {"y0": param_values["y0"] + 0.5, "a": -0.5 * param_values["a"]}

    @staticmethod
    def bound_param_uncertainties(
        param_values: Dict[str, float], param_uncertainties: Dict[str, float]
    ) -> Dict[str, float]:
        return {"y0": param_uncertainties["y0"], "a": 0.5 * param_uncertainties["a"]}

    @staticmethod
    def new_param_values(model_param_values: Dict[str, float]) -> Dict[str, float]:
        return {
            "y0": model_param_values["y0"] - 0.5,
            "a": -0.5 * model_param_values["a"],
        }
